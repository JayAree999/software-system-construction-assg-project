...
import io.muzoo.ssc.project.backend.util.OurUserDetailsService;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		//config security for REST backend APIs because we will not login using form login anymore.
		//disable csrf, normally you should not but life is a lot easier without it.

		http.csrf().disable();
		//Permit root and /api/login and /api/logout
		http.authorizeRequests()
				.antMatchers("/","/api/login", "/api/logout", "/api/whoami", "/api/register","/api/result_register","/api/todos").permitAll();
